...
export const api: ApiType = {
  server: {
    ping: async (): Promise<AxiosResponse<ApiResponse<PingResponse>>> =>
      await instance.get<ApiResponse<PingResponse>>("ping"),
  },
  auth: {
    login: async (data: LoginRequest): Promise<AxiosResponse<LoginResponse>> =>
      await instance.post<LoginResponse>("login", data),
    status: async (): Promise<AxiosResponse<TokenStatusResponse>> =>
      await instance.get<TokenStatusResponse>("token/status"),
    refresh: async (
      data: RefreshTokenRequest,
    ): Promise<AxiosResponse<LoginResponse>> =>
      await instance.post<LoginResponse>("token/refresh", data),
  },
  bookmark: {
    create: async (data): Promise<AxiosResponse> =>
      await instance.post("bookmark", data),
    update: async (id, data): Promise<AxiosResponse> =>
      await instance.patch(`bookmark/${id}`, data),
